...
public class EncryptHelper {
    ...
    private static Key generateKey(String key) {
        try {
            ...
            byte[] md5 = md.digest(key.getBytes(UTF_8));
    ...
    private static byte[] encodeDecode(int mode, byte[] bytes, String key) {
        try {
            ...
            cipher.init(mode, generateKey(key), new IvParameterSpec(HEX.getBytes(UTF_8)));
    ...
    public static String encode(String blankText, String key) {
        ...
        try {
            byte[] bytes = encodeDecode(Cipher.ENCRYPT_MODE, blankText.getBytes(UTF_8), key);
